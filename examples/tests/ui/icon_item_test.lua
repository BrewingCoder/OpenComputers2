-- icon_item_test.lua -- Icon shape="item" visual check.
--
-- Six icons in a row, each rendering a real MC item texture via the
-- new drawItem primitive. Verifies:
--  - item textures render above the text layer
--  - 20:9 pxbuf aspect is corrected so icons render visually square
--  - lazy clearIcons emits exactly once per render pass
--  - wide widget (shrink width) vs tall widget (shrink height) both center
--  - missing / bogus item IDs fall back to AIR (no icon, no crash)
