-- /rom/examples/ui/container.lua -- visual showcase for ui_v1
-- layout containers: VBox, HBox, Spacer, Card.
--
--   run /rom/examples/ui/container.lua
--
-- Setup: computer wired to a 2x2 (or larger) monitor group. Renders a
-- labeled grid of layout demonstrations.
--
-- Covers every container prop:
--   [1] VBox + HBox with fixed-size children
--   [2] Spacer eating remainder (VBox + HBox)
--   [3] Flex weights 1:3 proportional split
--   [4] padding + gap on a VBox
--   [5] Card default chrome (bg + border + padding)
--   [6] Card with custom bg + no border
--   [7] Nested HBox inside VBox
--   [8] visible=false skipped in layout
--   [9] Deeply nested tree with set() mutation
...
local function section(x, y, w, text)
...
-- [7] Nested HBox inside Card inside VBox
...
local mutCard = ui.Card{ x=4, y=346, width=340, height=60, padding=2, bg="bgCard",
    children={
...
-- After prop change, re-run layout so children re-inherit the new inner rect.
mutCard:layout()
ui.render()
