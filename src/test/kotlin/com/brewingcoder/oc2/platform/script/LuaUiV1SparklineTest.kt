...
/**
 * Unit tests for `ui_v1`'s Sparkline widget (Lua). Mirror of [JsUiV1SparklineTest].
 */
class LuaUiV1SparklineTest {
    ...
    fun `Sparkline constructor applies defaults`() {
        ...
        val r = CobaltLuaHost().eval(
            ...
            local s = ui.Sparkline{}
            print(s.kind .. " " .. s.x .. " " .. s.y .. " " .. s.width .. " " .. s.height)
            print(s.capacity .. " " .. tostring(s.min) .. " " .. tostring(s.max))
            print(s.color .. " " .. s.bg .. " " .. s.border)
            print(tostring(s.baseline) .. " " .. s.baselineColor)
            print(tostring(s.fill) .. " " .. tostring(s.showLast) .. " " .. tostring(s.visible))
    ...
    fun `Sparkline ctor applies props`() {
        ...
        val r = CobaltLuaHost().eval(
            ...
            print(tostring(s.fill) .. " " .. s.fillColor .. " " .. tostring(s.showLast))
            print("values=" .. #s.values .. " first=" .. s.values[1] .. " last=" .. s.values[3])
            """.trimIndent(),
            "test.lua", env,
        )
        r.ok shouldBe true
    ...
    fun `Sparkline push appends to ring`() {
        ...
        val r = CobaltLuaHost().eval(
            """
            local ui = require("ui_v1")
            local s = ui.Sparkline{ capacity=10 }
            s:push(1); s:push(2); s:push(3)
            print(#s.values .. " " .. s.values[1] .. " " .. s.values[3])
            """.trimIndent(),
            "test.lua", env,
        )
        r.ok shouldBe true
    ...
    fun `Sparkline push rings off head when over capacity`() {
        ...
        val r = CobaltLuaHost().eval(
            """
            local ui = require("ui_v1")
            local s = ui.Sparkline{ capacity=3 }
            s:push(1); s:push(2); s:push(3); s:push(4); s:push(5)
            print(#s.values .. " " .. s.values[1] .. " " .. s.values[2] .. " " .. s.values[3])
            """.trimIndent(),
            "test.lua", env,
        )
        r.ok shouldBe true
    ...
    fun `Sparkline clear empties the ring`() {
        ...
        val r = CobaltLuaHost().eval(
            """
            local ui = require("ui_v1")
            local s = ui.Sparkline{ values={1,2,3} }
            s:clear()
            print(#s.values)
            """.trimIndent(),
            "test.lua", env,
    ...
    fun `Sparkline setValues replaces series`() {
        ...
        val r = CobaltLuaHost().eval(
            ...
            local s = ui.Sparkline{ values={1,2,3} }
            s:setValues({10, 20})
            print(#s.values .. " " .. s.values[1] .. " " .. s.values[2])
            """.trimIndent(),
            "test.lua", env,
        )
        r.ok shouldBe true
    ...
    fun `Sparkline ctor defensive copy does not alias input array`() {
        // Mutating the caller's array after ctor must not leak into the widget.
        val (env, out) = mkEnv()
        val r = CobaltLuaHost().eval(
            """
            local ui = require("ui_v1")
            local src = {1,2,3}
            local s = ui.Sparkline{ values=src }
            src[1] = 999
            print(s.values[1])
            """.trimIndent(),
            "test.lua", env,
    ...
    @Test
    fun `Sparkline with two values draws at least one drawLine for the line segment`() {
        val monitor = RecordingMonitor()
        val (env, _) = mkEnv(monitor)
        val r = CobaltLuaHost().eval(
            """
    ...
    fun `Sparkline fill adds vertical drawLines under every sample`() {
        ...
        r.ok shouldBe true
        // 3 fill columns + 2 segments between samples = 5 drawLine calls minimum.
