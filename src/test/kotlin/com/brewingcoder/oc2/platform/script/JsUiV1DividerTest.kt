...
/**
 * Unit tests for `ui_v1`'s Divider widget (JS). Mirror of [LuaUiV1DividerTest]
 * to guarantee API symmetry across script languages.
 */
class JsUiV1DividerTest {
    ...
    fun `Divider constructor applies defaults`() {
        ...
        val r = RhinoJSHost().eval(
            ...
            print(d.orientation + " " + d.color + " " + d.thickness + " " + d.visible);
            """.trimIndent(),
            "test.js", env,
        )
        r.ok shouldBe true
    ...
    fun `Divider hex color overrides theme`() {
        ...
        val r = RhinoJSHost().eval(
            ...
            var ui = require("ui_v1");
            ui.mount(peripheral.find("monitor"),
                     ui.Divider({ x:0, y:0, width:50, height:4, color:0xFFABCDEF|0 }));
            ui.render();
            """.trimIndent(),
            "test.js", env,
        )
