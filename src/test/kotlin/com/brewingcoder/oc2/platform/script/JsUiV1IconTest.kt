...
/**
 * Unit tests for `ui_v1`'s Icon widget (JS). Mirror of [LuaUiV1IconTest].
 */
class JsUiV1IconTest {
    ...
    fun `Icon ctor defensive copy does not alias bits array`() {
        ...
        val r = RhinoJSHost().eval(
            ...
            var ui = require("ui_v1");
            var src = [[1,0],[0,1]];
            var i = ui.Icon({ shape:"bits", bits:src });
            src[0][0] = 999;
            print(i.bits[0][0] + " " + i.bits[1][1]);
            """.trimIndent(),
            "test.js", env,
        )
        r.ok shouldBe true
